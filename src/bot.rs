...
pub async fn on_stripped_state_member(
    ...
    {
        accept_invite(&event, &room, &config).await.unwrap();
    }
}
...
async fn check_member(
    ...
    {
        let mut events = get_user_rules(&event, &room, &client).await?;
        ...
        let mut events = get_server_rules(&event, &room, &client).await?;
        events.sort_by_key(|e| e.content.action);
        if let Some(event) = events.first() {
            apply_rule(
                &room,
...
async fn get_user_rules(
    ...
    for r in mock_room_list() {
        ...
        let events_inner: Vec<SyncStateEvent<UserRuleEventContent>> = events_inner
            ...
            .filter(|e| {
                is_match_user_rule(
                    UserId::try_from(event.state_key.clone()).unwrap(),
                    &e.content,
                )
                .unwrap()
            })
...
>>>>>>> 72333a3 (refactor: Adopt new event handler abstractions)
}
